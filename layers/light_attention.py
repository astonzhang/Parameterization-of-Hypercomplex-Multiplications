...
def compute_attention_component(antecedent,
  ...
  if filter_width == 1:
      if(mode=='quaternion'):
          ...
          return octonion_ffn_3d(antecedent, total_depth, name=name,
            init=tf.random_normal_initializer(stddev=initializer_stddev))
      elif(mode=='factor'):
          return factorized_ffn_3d(antecedent, total_depth, name=name,
            init=tf.random_normal_initializer(stddev=initializer_stddev))
      elif(mode=='random'):
          return random_ffn_3d(antecedent, total_depth, name=name,
                init=tf.random_normal_initializer(stddev=initializer_stddev), n=factor,
                dual=dual)
      elif(mode=='fm'):
          _, x = build_fm(antecedent, k=total_depth, name=name,
                initializer=tf.random_normal_initializer(stddev=initializer_stddev),
                reshape=True
                )
          return x
          ...
          if('v' in name):
              print('Retaining linear transform for Values')
              return common_layers.dense(
                    antecedent, total_depth, use_bias=False, name=name)
          else:
              x, _ = build_tensor_machines(antecedent, total_depth, k=factor,
                    name=name,
                    initializer=tf.random_normal_initializer(stddev=initializer_stddev),
                    reshape=True)
              return x
...
def compute_kv(query_antecedent,
            memory_antecedent,
            total_key_depth,
            total_value_depth,
            q_filter_width=1,
...
def compute_qkv(query_antecedent,
  ...
  if(num_extra_outputs>0):
      output = [k, v]
      for i in range(num_extra_outputs):
          v = compute_attention_component(
              memory_antecedent,
              total_value_depth,
              kv_filter_width,
...
def multihead_attention(query_antecedent,
  ...
  """
  print('{}:{}'.format(att_mode, attention_type))
  if(attention_type=="compact_v2"):
      total_key_depth = hparams.mini_dim * num_heads
      query_antecedent= query_antecedent[:,:hparams.max_length+1,:]
      if(memory_antecedent is not None):
          memory_antecedent = memory_antecedent[:,:hparams.max_length+1,:]

  if(light_mode=='ftm'):
      total_key_depth = num_heads * hparams.mini_dim
      # total_value_depth = num_heads * 4
  if(hparams.upsize!=0):
      print("[Old] Key={} Value={}".format(total_key_depth, total_value_depth))
      total_key_depth *= hparams.upsize
      total_value_depth *= hparams.upsize
      total_key_depth = int(total_key_depth)
      total_value_depth = int(total_value_depth)
  ...
  with tf.variable_scope(name, default_name="multihead_attention",
    ...
    if cache is None or memory_antecedent is None:
      if("memchain" in attention_type):
          q, v = compute_qkv(query_antecedent, memory_antecedent,
                            total_key_depth, total_value_depth, q_filter_width,
                            kv_filter_width, q_padding, kv_padding,
                            vars_3d_num_heads=vars_3d_num_heads,
                            mode=light_mode, num_extra_outputs=1)
          k = v[0]
      else:
          q, k, v = compute_qkv(query_antecedent, memory_antecedent,
                                ...
                                kv_filter_width, q_padding, kv_padding,
                                vars_3d_num_heads=vars_3d_num_heads,
                                mode=light_mode, factor=hparams.factor, dual=hparams.dual)
    if cache is not None:
      print("USING CACHE")
      print(att_mode)
      if attention_type not in ["dot_product", "dot_product_relative",
                            "quaternion_dot_product", "linear_v1"]:
      ...
      if memory_antecedent is not None:
        ...
        q = compute_attention_component(query_antecedent, total_key_depth,
                                        ...
                                        vars_3d_num_heads=vars_3d_num_heads,
                                        mode=light_mode, factor=hparams.factor,
                                        dual=hparams.dual)
        k = cache["k_encdec"]
        v = cache["v_encdec"]
      else:
        k = split_heads(k, num_heads, light_mode=hparams.light_mode)
        v = split_heads(v, num_heads, light_mode=hparams.light_mode)
    ...
    q = split_heads(q, num_heads, light_mode=light_mode)
    if cache is None:
      if 'memchain' in attention_type:
          v = [split_heads(x, num_heads, light_mode=light_mode) for x in v]
      else:
          k = split_heads(k, num_heads, light_mode=light_mode)
          v = split_heads(v, num_heads, light_mode=light_mode)
    ...
    if attention_type == 'linear_v2':
        x = linear_attention_v2(q,
                                k,
                                v,
                                bias,
                                dropout_rate,
                                ...
                                mini_dim=hparams.mini_dim,
                                num_slots=hparams.num_slots,
                                hparams=hparams,
                                att_mode=att_mode,
                                simple=True)
    elif attention_type == 'linear_v1':
        x = linear_attention_v1(q,
                                k,
                                v,
                                bias,
                                dropout_rate,
                                ...
                                att_mode=att_mode,
                                simple=True)
    elif attention_type == 'linear_v3':
        x = linear_attention_v3(q,
                                k,
                                v,
                                bias,
                                dropout_rate,
                                ...
                                att_mode=att_mode,
                                simple=True)
    elif attention_type == 'linear_v2_rel':
        x = linear_attention_v2(q,
                                k,
                                v,
                                bias,
                                ...
                                hparams=hparams,
                                att_mode=att_mode,
                                simple=True, relational=True)
    elif attention_type == 'linear_v2_cond':
        x = linear_attention_v2(q,
                                k,
                                v,
                                bias,
                                ...
                                hparams=hparams,
                                att_mode=att_mode,
                                simple=True, conditioned=True)
    elif attention_type == 'compact_simple':
        x = compact_attention(q,
                            k,
                            v,
                            bias,
                            dropout_rate,
                            ...
                            att_mode=att_mode,
                            simple=True)
    elif attention_type == 'compact_v2':
        x = compact_attention_v2(q,
                            k,
                            v,
                            bias,
                            dropout_rate,
        ...
        x = compact_attention(q,
                            ...
                            att_mode=att_mode,
                            simple=False)
      # _k = tf.reshape(_k, [-1, total_key_depth])
      ...
      x = dot_product_attention(q, k, v, bias, dropout_rate, image_shapes,
                                ...
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif attention_type == "focus":
      if not vars_3d:
         q *= key_depth_per_head**-0.5
      x = dot_product_attention_focus(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
      ...
      if not vars_3d:
         q *= key_depth_per_head**-0.5
      x = dot_product_attention_block(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims,
                                hparams=hparams)
    elif attention_type == "ftm":
      x = dot_product_attention_ftm(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif attention_type == 'bimap':
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        x = dot_product_attention_bimap(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif attention_type == 'trimap':
        x = dot_product_attention_trimap(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif 'memchain' in attention_type:
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        if('fixed' in attention_type):
            mode = 'fixed'
        elif('ffn' in attention_type):
            mode = 'ffn'
        elif('lc' in attention_type):
            mode = 'lc'
        else:
            mode = ''
        if('uq' in attention_type):
            compute = 'unique'
        else:
            compute = 'all'
        x = dot_product_attention_ms(q, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims,
                                mode=mode, compute=compute)
    elif attention_type == 'smap':
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        x = dot_product_attention_self_bimap(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif attention_type == 'rmap':
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        x = dot_product_attention_residual_bimap(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims, res=res)
    elif attention_type == 'amap':
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        x = dot_product_attention_aggressive_bimap(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims, res=res)
    elif attention_type == 'dpflow':
        if not vars_3d:
          q *= key_depth_per_head**-0.5
        x = dense_permutation_flow_attention(q, k, v, bias, dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims, res=res)
    elif attention_type == 'dmap':
        print("Dense Multi-way Attention")
        # kv_store = [k, v]
        num_permute = 2
        # if(memory_antecedent is None):
        #     memory_antecedent = query_antecedent
        # kv_store = compute_attention_component(
        #                   memory_antecedent,
        #                   total_value_depth * num_permute, name="kv",
        #                   filter_width=1,
        #                    mode=hparams.light_mode)
        if not vars_3d:
          q *= key_depth_per_head**-0.5

        # kv_store = split_heads(kv_store, num_heads, light_mode=hparams.light_mode)
        l = tf.shape(k)[2]
        dim = total_key_depth // num_heads
        print("====================")
        print(att_mode)
        # print(bias)
        ...
        kv_store = tf.concat([k, v], 2)
        kv_store = tf.reshape(kv_store, [-1, num_heads, num_permute, l, key_depth_per_head])
        x = dense_multiway_permuted_attention(q, kv_store, bias,
                                dropout_rate, image_shapes,
                                save_weights_to=save_weights_to,
                                make_image_summary=make_image_summary,
                                dropout_broadcast_dims=dropout_broadcast_dims)
    elif attention_type == 'shake_dot_product':
        print("Using Shake Dot Product Attention...")
        x = shake_dot_product_attention(q, k, v, bias, dropout_rate, image_shapes,
                                  save_weights_to=save_weights_to,
                                  make_image_summary=make_image_summary,
                                  dropout_broadcast_dims=dropout_broadcast_dims,
      ...
      x = dilated_self_attention_1d(q, k, v, block_length, block_width,
                                    gap_size, num_memory_blocks)
    x = combine_heads(x, light_mode=light_mode)
    ...
    if vars_3d:
      ...
      x = tf.tensordot(x, o_var, axes=1)
        ...
        if('quaternion' in light_mode):
            x = quarternion_ffn_3d(x, output_depth, name='output_transfom',
                                init=tf.random_normal_initializer(
                                stddev=output_depth ** -0.5))
        elif('factor' in light_mode):
            x = factorized_ffn_3d(x, output_depth, name='output_transform',
                                init=tf.random_normal_initializer(
                                stddev=output_depth ** -0.5))
        elif('random' in light_mode):
            x = random_ffn_3d(x, output_depth, name='output_transform',
                                init=tf.random_normal_initializer(
                                stddev=output_depth ** -0.5), n=hparams.factor)
        else:
...
@expert_utils.add_name_scope()
def split_heads(x, num_heads, light_mode='None'):
...
@expert_utils.add_name_scope()
def combine_heads(x, light_mode='None'):
...
def grouped_attention_multihead(query_antecedent,
  ...
  with tf.variable_scope(
    ...
    kv = common_layers.dense(
        ...
        use_bias=False,
        name="kv_transform")
    q = split_heads(q, num_heads, mode=hparams.light_mode)
    kv = split_heads(kv, num_heads, mode=hparams.light_mode)
    ...
    q_pred = common_layers.dense(
        ...
        use_bias=False,
        name="q_pred")
    q_pred = split_heads(q_pred, num_heads, mode=hparams.light_mode)
    ...
    m_pred = common_layers.dense(
        ...
        use_bias=False,
        name="m_pred")
    m_pred = split_heads(m_pred, num_heads, mode=hparams.light_mode)
...
def shake_dot_product_attention(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  with tf.variable_scope(
    ...
    # implements shake-shake attention dropout
    if(is_training):
        rand_forward = tf.random_uniform(
                  tf.shape(logits), minval=0.1, maxval=1, dtype=tf.float32)
        rand_backward = tf.random_uniform(
              tf.shape(logits), minval=0.1, maxval=1, dtype=tf.float32)
        x = logits
...
def compact_attention(q,
                      ...
                      att_mode='',
                      simple=False):
    print("Compress Linear Attention {}".format(simple))
    qlen = tf.shape(q)[2]
    klen = tf.shape(k)[2]
    num_heads = q.get_shape().as_list()[1]
    _k = k
    _k =  common_layers.dense(_k, mini_dim,
                      use_bias=False, name="compress")
    bsz = tf.shape(_k)[0]
    _k = tf.reshape(_k, [bsz, num_heads, -1])
    padding = tf.zeros([bsz, num_heads, (hparams.max_length-klen) * mini_dim],
                      dtype=tf.float32)
    _k = tf.concat([_k, padding], 2)
    _k = tf.reshape(_k, [bsz, num_heads, 1, hparams.max_length * mini_dim])

    _k = tf.tile(_k, [1, 1, qlen, 1])
    if(att_mode=='self_causal'):
        print("Adding Causal Mask..")
        band = common_layers.ones_matrix_band_part(
            qlen,
            hparams.max_length,
            -1,
            0,
            out_shape=[1, 1, qlen, hparams.max_length])
        mask = tf.tile(band, [1, 1, 1, mini_dim])
        mask = (1.0 - mask)
        _k *= mask

    qk = tf.concat([q, _k], 3)
    if(simple):
        qk = common_layers.dense(qk, 1,
                          use_bias=False,
                          name="compress2")
        qk = qk[:,:,:,:klen]
        qk = tf.nn.softmax(qk)
        ...
        qk = common_layers.dense(qk, hparams.max_length,
                          use_bias=False,
                          name="compress2")
        qk = qk[:,:,:,:klen]
        qk = tf.nn.softmax(qk)
...
def build_tensor_machines(input_vec, num_outputs=5, k=5,
                    reuse=None, name='', initializer=None,
                    reshape=False, linear=False, out_proj=True):
    """ Tensor Machine Layer
    output is not scalar but multiple scalar
    """
    print("Building Tensor Machine layer..n={} k={}".format(num_outputs, k))

    # Factorization Machine
    dim = input_vec.get_shape().as_list()[2]
    if(initializer is None):
        # initializer = tf.random_normal_initializer(0, dim ** -0.5)
    ...
    with tf.variable_scope("fm_{}".format(name), reuse=reuse) as scope:
        ...
        # fm_w0 = tf.get_variable("fm_w0", [1],
        #             initializer=tf.constant_initializer([0]))
        # fm_w = tf.get_variable("fm_w", [fm_p * num_outputs],
        #             initializer = tf.constant_initializer([0]))
        ...
        #                              tf.expand_dims(fm_w, 1))
        if(linear):
            fm_linear = common_layers.dense(input_vec, num_outputs,
                            use_bias=False,
                            name="fm_linear")
        ...
        # print(fm_interactions)
        latent_dim = fm_interactions
        fm_interactions = tf.reshape(fm_interactions, [-1, num_outputs, k])

        fm_interactions = tf.reduce_sum(
                                fm_interactions, 2)
        # (bsz x seq_len) x num_outputs

        fm_interactions = tf.multiply(0.5, fm_interactions)
        fm_prediction = fm_interactions
        if(linear):
            fm_prediction = tf.add(fm_linear, fm_interactions)

        if(out_proj):
            fm_prediction = common_layers.dense(fm_prediction, num_outputs,
                                    use_bias=False,
                                    name="out_proj")
        fm_prediction = tf.reshape(fm_prediction,
                        [-1, seq_lens, num_outputs])


        return fm_prediction, fm_prediction

def compact_attention_v2(q,
                      k,
                      v,
                      bias,
                      dropout_rate=0.0,
                      ...
                      att_mode='',
                      simple=False):
    print("Compress Linear Attention V2 {}".format(simple))

    q = q[:,:,:hparams.max_length,:]
    k = k[:,:,:hparams.max_length,:]
    v = v[:,:,:hparams.max_length,:]
    bias = bias[:,:,:,:hparams.max_length]
    qlen = tf.shape(q)[2]
    klen = tf.shape(k)[2]
    dim = v.get_shape().as_list()[3]
    num_heads = q.get_shape().as_list()[1]
    # qlen = tf.clip_by_value(qlen, 1, hparams.max_length)
    # klen = tf.clip_by_value(klen, 1, hparams.max_length)
    # bsz = tf.shape(k)[0]
    # bsz = q.get_shape().as_list()[0]
    bsz = tf.shape(q)[0]
    _k = tf.reshape(k, [bsz, num_heads, -1])
    _q = tf.reshape(q, [bsz, num_heads, -1])

    len1 = tf.clip_by_value(hparams.max_length-klen, 0, hparams.max_length-1)
    len2 = tf.clip_by_value(hparams.max_length-qlen, 0, hparams.max_length-1)

    padding = tf.zeros([bsz, num_heads, len1 * mini_dim], dtype=tf.float32)
    padding2 = tf.zeros([bsz, num_heads, len2 * mini_dim], dtype=tf.float32)

    _k = tf.concat([_k, padding], 2)
    _k = tf.reshape(_k, [bsz, num_heads, hparams.max_length * mini_dim])
    _q = tf.concat([_q, padding2], 2)
    _q = tf.reshape(_q, [bsz, num_heads, hparams.max_length * mini_dim])

    _q, _ = build_fm(_q, k=8, name='fm', reshape=True)
    _k, _ = build_fm(_k, k=8, name='fm', reshape=True, reuse=True)

    qk = tf.concat([_q, _k], 2)
    qk = tf.expand_dims(qk, 2)
    qk = tf.tile(qk, [1, 1, qlen, 1])
    # _k = tf.tile(_k, [1, 1, qlen, 1])
    # q = tf.concat([q, _k], 2)
    q = tf.concat([q, qk], 3)
    # bsz x num_heads x qlen x 5
    qproj = common_layers.dense(q, hparams.max_length,
                      use_bias=False, name="kproj")
    # bsz x num_heads x qlen x klen
    logits = qproj[:, :, :, :klen]
...
def dot_product_attention_match_only(q,
                      k,
                      bias,
                      dropout_rate=0.0,
                      image_shapes=None,
                      ...
                      dropout_broadcast_dims=None):
  """Dot-product attention.
  Only computes and returns logits
  """
  with tf.variable_scope(
      name, default_name="dot_product_attention", values=[q, k]) as scope:
    logits = tf.matmul(q, k, transpose_b=True)  # [..., length_q, length_kv]
    if bias is not None:
      bias = common_layers.cast_like(bias, logits)
      logits += bias
...
def dot_product_attention_block(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
  ...
  for h in [4,8,16]:
      _k = tf.reshape(k, [bsz * num_heads, klen, dim])
      k2 = tf.layers.average_pooling1d(_k, h, h, padding='same')
      pooled_len = tf.shape(k2)[1]
      k2 = tf.reshape(k2, [bsz, num_heads, pooled_len, dim])
      _logits = tf.matmul(q, k2, transpose_b=True)  # bsz x nh x qlen x (klen/h)
      _logits = tf.tile(_logits, [1, 1, 1, h])  # retile
      _logits = _logits[:,:,:,:klen]
      _logits = tf.reshape(_logits, [bsz, num_heads, qlen, klen])

      # proj = tf.layers.dense(k2, h) # bsz x nh x pooled_len x h
      # proj = tf.nn.softmax(proj)
      # proj = tf.reshape(proj, [bsz, num_heads, 1, pooled_len * h])
      # pos_bias = proj[:,:,:,:klen]

      pos_bias = tf.get_variable('h{}'.format(h),
                    [1, num_heads, 1, h])
      pos_bias = tf.tile(pos_bias, [1, 1, 1, pooled_len])
      pos_bias = pos_bias[:,:,:,:klen]
      # pos_bias = pos_bias[:, :, :qlen, :klen]
      _logits += pos_bias
      if bias is not None:
        _logits += bias
      _logits = tf.nn.softmax(_logits)
      logits += _logits
...
def dot_product_attention_focus(q,
                      k,
                      v,
                      bias,
                      dropout_rate=0.0,
  ...
  with tf.variable_scope(
      name, default_name="dot_product_attention", values=[q, k, v]) as scope:
    logits = tf.matmul(q, k, transpose_b=True)  # [..., length_q, length_kv]
    qrow = tf.layers.dense(q, 2)
    krow = tf.layers.dense(k, 2)
    a,b = tf.split(qrow, 2, axis=-1)
    c,d = tf.split(krow, 2, axis=-1)
    a = tf.squeeze(a, axis=-1)
    b = tf.squeeze(b, axis=-1)
    c = tf.squeeze(c, axis=-1)
    d = tf.squeeze(d, axis=-1)

    a = tf.cumsum(tf.nn.softmax(a))
    b = tf.cumsum(tf.nn.softmax(b))
    c = tf.cumsum(tf.nn.softmax(c))
    d = tf.cumsum(tf.nn.softmax(d))
    qfocus = a-b    # bsz x nh x qlen
    kfocus = c-d    # bsz x nh x klen
    qfocus = tf.expand_dims(qfocus, 2)
    kfocus = tf.expand_dims(kfocus, 2)
    logits *= qfocus
    logits *= kfocus
...
def dot_product_attention_ftm(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  with tf.variable_scope(
    ...
    ql = tf.shape(q)[2]
    kl = tf.shape(k)[2]
    num_heads = k.get_shape().as_list()[1]
    q2 = tf.tile(tf.expand_dims(q, 3), [1, 1, 1, kl, 1])
    k2 = tf.tile(tf.expand_dims(k, 2), [1, 1, ql, 1, 1])
    dim = k.get_shape().as_list()[3]
    qk = tf.concat([q2, k2], -1)
    qk = tf.reshape(qk, [-1, dim * 2])
    logits = common_layers.dense(
                qk, 1, use_bias=False, name=name)
    logits = tf.reshape(logits, [-1, num_heads, ql, kl])
...
def dense_multiway_permuted_attention(q,
                                kv,
                                bias,
                                dropout_rate=0.0,
                                image_shapes=None,
                                ...
                                save_weights_to=None,
                                dropout_broadcast_dims=None):
  """ kv is list of transformed vectors
  """
  num_heads = q.get_shape().as_list()[1]
  output = []

  num_permute = kv.get_shape().as_list()[2]

  # KV is bsz x num_heads x num_kv x seq_len x dim
  klen = tf.shape(kv)[-2]
  dim = q.get_shape().as_list()[3]
  # v = tf.expand_dims(kv, 2) # bsz x num_heads x 1 x num_kv x seq_len x dim
  # k = tf.expand_dims(kv, 3) # bsz x num_heads x num_kv x 1 x seq_len x dim
  v = tf_repeat(kv, [1, 1, num_permute, 1, 1])
  k = tf.tile(kv, [1, 1, num_permute, 1, 1])
  # bias is 1 x 1 x 1 x L1 x L2
  # bias = tf.expand_dims(bias, 3)
  v = tf.reshape(v, [-1, num_heads * (num_permute ** 2), klen, dim])
  k = tf.reshape(k, [-1, num_heads * (num_permute ** 2), klen, dim])
  # kv = tf.tile(tf.expand_dims(kv, 2), [1, 1, num_permute, 1, 1, 1])
  qlen = tf.shape(q)[2]

  # q = tf.reshape(q, [-1, num_heads, 1, qlen, dim])
  q = tf.tile(q, [1, num_permute ** 2, 1, 1])

  logits = tf.matmul(q, k, transpose_b=True)
  #print(logits)    # bsz x num_heads x num_kv x 1 x qlen x klen
  #print(bias)
  if bias is not None:
    bias = common_layers.cast_like(bias, logits)
    logits += bias
  logits = tf.nn.softmax(logits)
  # print(dropout_broadcast_dims)
  logits = common_layers.dropout_with_broadcast_dims(
      logits, 1.0 - dropout_rate, broadcast_dims=dropout_broadcast_dims)
  # logits = tf.reshape(logits, [-1, num_heads * (num_permute ** 2, qlen, klen])
  x = tf.matmul(logits, v)

  x = tf.reshape(x, [-1, num_heads, num_permute **2, qlen, dim])
  # alpha = tf.get_variable('alpha', [1, num_heads, 1, 1, num_permute **2])
  # alpha = tf.nn.softmax(alpha)
  # alpha = tf.transpose(alpha, [0, 1, 4, 3, 2])
  x = tf.reduce_max(x, 2)
  return x

def dot_product_attention_bimap(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
                            ...
                            dropout_broadcast_dims=None):
  num_heads = q.get_shape().as_list()[1]
  x1 = dot_product_attention(q, k, v, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  x2 = dot_product_attention(q, v, k, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  alpha = tf.get_variable('alpha', [1, num_heads, 1, 1])
...
def dot_product_attention_trimap(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
  ...
  s = dim ** -0.5
  x1 = dot_product_attention(q * s, k, v, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  x2 = dot_product_attention(q * s, v, k, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  # bias should be bsz x 1 x 1 x qlen
  print(bias)
  # tbias = tf.transpose(bias, [0,1,3,2])
  x3 = dot_product_attention(k * s, v, q, None, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)

  alpha = tf.get_variable('alpha', [1, num_heads, 1, 1, 3])
  alpha = tf.nn.softmax(alpha)
  x1 = tf.expand_dims(x1, -1)
  x2 = tf.expand_dims(x2, -1)
  x3 = tf.expand_dims(x3, -1)
  x = tf.concat([x1, x2, x3], -1)
  x = tf.reduce_sum(x * alpha, -1)
  return x

def dot_product_attention_ms(q,
                            output,
                            bias,
                            dropout_rate=0.0,
                            image_shapes=None,
                            ...
                            save_weights_to=None,
                            dropout_broadcast_dims=None,
                            mode='headwise', compute='all'):
  num_heads = q.get_shape().as_list()[1]
  all_output = []
 # l = q.get_shape().as_list()[2]
  l = tf.shape(q)[2]
  d = q.get_shape().as_list()[3]
  ...
  if(compute=='unique'):
      for i in range(len(output)):
          for j in range(len(output)):
              if(i==j):
                  continue
              k, v = output[i], output[j]
              x = dot_product_attention(q, k, v, bias, dropout_rate=dropout_rate,
                                        dropout_broadcast_dims=dropout_broadcast_dims)
              all_output.append(tf.expand_dims(x, -1))
  else:
      for i in range(len(output)):
          for j in range(len(output)):
              k, v = output[i], output[j]
  ...
  num_vals = len(all_output)
  if(mode=='fixed'):
      alpha = tf.get_variable('alpha', [1, 1, 1, 1, num_vals])
      alpha = tf.nn.softmax(alpha)
      all_output = tf.concat(all_output, -1)
      x = tf.reduce_sum(all_output * alpha, -1)
  elif(mode=='ffn'):
      all_output = tf.concat(all_output, -1)
      all_output = tf.reshape(all_output, [-1, num_heads,
                            l, (num_vals ** 2) * d])
      x = common_layers.dense(all_output, d, use_bias=True)
  elif(mode=='lc'):
      alpha = tf.get_variable('alpha', [1, num_heads, 1, 1, num_vals])
      all_output = tf.concat(all_output, -1)
      x = tf.reduce_sum(all_output * alpha, -1)
  else:
      alpha = tf.get_variable('alpha', [1, num_heads, 1, 1, num_vals])
...
def dot_product_attention_residual_bimap(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
                            ...
                            make_image_summary=True,
                            save_weights_to=None,
                            dropout_broadcast_dims=None, res=None,
                            mash=False):
  num_heads = q.get_shape().as_list()[1]
  x1 = dot_product_attention(q, k, v, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  ...
  if(res is not None):
      print("Residual")
      x3 = dot_product_attention(q, res[1], res[2], bias, dropout_rate=dropout_rate,
                                dropout_broadcast_dims=dropout_broadcast_dims)
      x4 = dot_product_attention(q, res[2], res[1], bias, dropout_rate=dropout_rate,
                                dropout_broadcast_dims=dropout_broadcast_dims)
      alpha = tf.get_variable('alpha2', [1, num_heads, 1, 1, 4])
      alpha = tf.nn.softmax(alpha)
      x1 = tf.expand_dims(x1, -1)
      x2 = tf.expand_dims(x2, -1)
      x3 = tf.expand_dims(x3, -1)
...
def dot_product_attention_aggressive_bimap(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
                            ...
                            dropout_broadcast_dims=None, res=None,
                            mash=False):
  """ This version computes extreme permutations
  ...
  if(res is not None):
      ...
      # x4 = dot_product_attention(q, res[2], res[1], bias, dropout_rate=dropout_rate,
      #                           dropout_broadcast_dims=dropout_broadcast_dims)
      x5 = dot_product_attention(q, k, res[2], bias, dropout_rate=dropout_rate,
                                dropout_broadcast_dims=dropout_broadcast_dims)
      x6 = dot_product_attention(q, v, res[1], bias, dropout_rate=dropout_rate,
                                dropout_broadcast_dims=dropout_broadcast_dims)
      # x7 = dot_product_attention(q, res[1], k, bias, dropout_rate=dropout_rate,
      #                           dropout_broadcast_dims=dropout_broadcast_dims)
      # x8 = dot_product_attention(q, res[2], v, bias, dropout_rate=dropout_rate,
      #                           dropout_broadcast_dims=dropout_broadcast_dims)
      alpha = tf.get_variable('alpha2', [1, 1, 1, 1, 4])
      # alpha = gumbel_softmax(alpha, 0.5)
      alpha = tf.nn.softmax(alpha)
      x1 = tf.expand_dims(x1, -1)
      x2 = tf.expand_dims(x2, -1)
      # x3 = tf.expand_dims(x3, -1)
      ...
      x = tf.concat([x1, x2, x5, x6], -1)
      # x = tf.reduce_mean(x, -1)
      x = tf.reduce_sum(x * alpha, -1)
  else:
      alpha = tf.get_variable('alpha', [1, 1, 1, 1, 2])
      alpha = tf.nn.softmax(alpha)
      # alpha = gumbel_softmax(alpha, 0.5)
      x1 = tf.expand_dims(x1, -1)
      x2 = tf.expand_dims(x2, -1)
...
def dense_permutation_flow_attention(q,
                            k,
                            v,
                            bias,
                            dropout_rate=0.0,
                            ...
                            save_weights_to=None,
                            dropout_broadcast_dims=None, res=None,
                            mash=False, fix_heads=True):
  """ Maybe the final version?
  """
  num_heads = q.get_shape().as_list()[1]

  qk = dot_product_attention_match_only(q, k, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  qv = dot_product_attention_match_only(q, v, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  x1 = tf.matmul(qk, v)
  x2 = tf.matmul(qv, k)
  ...
  if(res is not None):
      print("Residual")
      print(res)
      x3 = tf.matmul(qk, res[2])
      x4 = tf.matmul(qv, res[1])
      alpha = tf.get_variable('alpha2', [1, n, 1, 1, 4])
      # alpha = gumbel_softmax(alpha, 0.5)
      alpha = tf.nn.softmax(alpha)
      x1 = tf.expand_dims(x1, -1)
      x2 = tf.expand_dims(x2, -1)
      ...
      alpha = tf.get_variable('alpha', [1, n, 1, 1, 2])
      alpha = tf.nn.softmax(alpha)
      # alpha = gumbel_softmax(alpha, 0.5)
      x1 = tf.expand_dims(x1, -1)
      x2 = tf.expand_dims(x2, -1)
...
def dot_product_attention_self_bimap(q,
                                    k,
                                    v,
                                    bias,
                                    dropout_rate=0.0,
  ...
  x2 = dot_product_attention(q, v, k, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  x3 = dot_product_attention(q, k, k, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)
  x4 = dot_product_attention(q, v, v, bias, dropout_rate=dropout_rate,
                            dropout_broadcast_dims=dropout_broadcast_dims)

  alpha = tf.get_variable('alpha', [1, num_heads, 1, 1], initializer=tf.zeros_initializer)
  beta = tf.get_variable('beta', [1, num_heads, 1, 1], initializer=tf.zeros_initializer)
  gamma = tf.get_variable('gamma', [1, num_heads, 1, 1], initializer=tf.zeros_initializer)

  v = ((1-alpha) * x1) + (alpha * x4)
  k = ((1-beta) * x2) + (beta * x3)
...
def deep_dot_product_attention(q,
                      k,
                      v,
                      bias,
                      dropout_rate=0.0,
                      ...
                      mini_dim=4,
                      hparams=None,
                      att_mode='', k_vals=[], v_vals=[]):
  print("Deeper Dot Product Attention")
  k_vals = [k] + k_vals
  v_vals = [v] + v_vals
  num_heads = q.get_shape().as_list()[1]
  dim = q.get_shape().as_list()[3]
  ...
  for i in range(len(k_vals)):
      # number of key-vals
      _k, _v = k_vals[i], v_vals[i]
      q = dot_product_attention(q, _k, _v, bias, dropout_rate, image_shapes,
                                  save_weights_to=save_weights_to,
                                  make_image_summary=make_image_summary,
                                  dropout_broadcast_dims=dropout_broadcast_dims)
      # outputs.append(tf.expand_dims(q, 4))
  x = q
  # context = tf.get_variable('context',[1, num_heads, 1, dim, 1],
  #                     initializer=init)
  # bsz = tf.shape(q)[0]
  # context = tf.tile(context, [bsz, 1, 1, 1, 1])
  # outputs = tf.concat(outputs, 4)
  # # bsz x num_heads x seq_len x dim x depth
  # att = tf.reduce_sum(context * outputs, 3, keepdims=True)
  # att = tf.nn.softmax(att)
  # x = tf.reduce_sum(outputs * att, 4)
  return x

def linear_attention_v2(q,
                        k,
                        v,
                        bias,
                        dropout_rate=0.0,
                        ...
                        hparams=None,
                        att_mode='',
                        simple=False, num_slots=5,
                        relational=False,
                        conditioned=False,
                        conv=False):
  print("Linear Attention V2 {} {}".format(simple, att_mode))
  qlen = tf.shape(q)[2]
  klen = tf.shape(k)[2]
  vlen = tf.shape(v)[2]
  num_heads = q.get_shape().as_list()[1]
  ...
  dim = q.get_shape().as_list()[3]
  # _k =  common_layers.dense(_k, dim,
    #                   use_bias=False, name="compress")
  init = tf.random_normal_initializer(
      0, dim**-0.5)
  context = tf.get_variable('context_key',[1, num_heads, dim, num_slots],
                    initializer=init)
  context2 = tf.get_variable('context_val',[1, num_heads, dim, num_slots],
                    initializer=init)
  bsz = tf.shape(q)[0]
  context = tf.tile(context, [bsz, 1, 1, 1])
  context2 = tf.tile(context2, [bsz, 1, 1, 1])

  # conditioned
  if(conditioned):
      max_q = tf.expand_dims(tf.reduce_max(q, 2), 2)
      mean_q = tf.expand_dims(tf.reduce_mean(q, 2), 2)
      min_q = tf.expand_dims(tf.reduce_min(q, 2), 2)
      condition = [max_q, mean_q, min_q]
      conditioned_context = tf.concat(condition, 3)
      conditioned_context =  common_layers.dense(conditioned_context, dim,
                          use_bias=False, name="conditioned")
      conditioned_context = tf.transpose(conditioned_context, [0, 1, 3,2])

      context = tf.concat([context, conditioned_context], 3)
      context2 = tf.concat([context2, conditioned_context], 3)
      num_slots +=1

  k_softmax = tf.matmul(k * (dim **-0.5), context)    # bsz x num-heads, seq_len, num_slots
  v_softmax = tf.matmul(v * (dim **-0.5), context2)
  k_softmax = tf.reshape(k_softmax, [-1, num_heads, num_slots, klen])
  v_softmax = tf.reshape(v_softmax, [-1, num_heads, num_slots, vlen])
  # if(att_mode=='self'):
  k_softmax += bias
  v_softmax += bias
  print(bias)
  k_softmax = tf.nn.softmax(k_softmax)
  v_softmax = tf.nn.softmax(v_softmax)
    # k is bsz x num_heads, seq_len, dim
  k2 = tf.expand_dims(k, 2)     # bsz x num_heads x 1 x seq_len x dim
  k2 = tf.tile(k2, [1, 1, num_slots, 1, 1])
  v2 = tf.expand_dims(v, 2)
  v2 = tf.tile(v2, [1, 1, num_slots, 1, 1])

  k_softmax = tf.expand_dims(k_softmax, 4)
  v_softmax = tf.expand_dims(v_softmax, 4)
  k2 = k2 * k_softmax
  v2 = v2 * v_softmax
  k2 = tf.reduce_sum(k2, 3)
  v2 = tf.reduce_sum(v2, 3)
    # bsz x num_heads, num_slots, dim

  if(att_mode=='self'):
      bias_reshape = tf.transpose(bias, [0, 1, 3, 2])
  m = dot_product_attention(q * (dim **-0.5), k2, v2, None,
                              dropout_rate, image_shapes,
                              save_weights_to=save_weights_to,
                              make_image_summary=make_image_summary,
                              dropout_broadcast_dims=dropout_broadcast_dims)
  if(relational):
      print("Relational Linear-Time")
      for d in range(1):
          residual = m
          mk = common_layers.dense(m, dim,
                              use_bias=False, name="mk")
          mv = common_layers.dense(m, dim,
                              use_bias=False, name="mv")
          m = dot_product_attention(m, mk, mv, None,
                                  dropout_rate, image_shapes,
                                  save_weights_to=save_weights_to,
                                  make_image_summary=make_image_summary,
                                  dropout_broadcast_dims=dropout_broadcast_dims)
  ...
  x = dot_product_attention(m * (dim **-0.5), k, v, bias, dropout_rate, image_shapes,
                              save_weights_to=save_weights_to,
                              make_image_summary=make_image_summary,
                              dropout_broadcast_dims=dropout_broadcast_dims)
  return x

def linear_attention_v1(q,
                        k,
                        v,
                        bias,
                        dropout_rate=0.0,
                        ...
                        conditioned=False,
                        conv=False):
  print("Linear Attention V1 {} {}".format(simple, att_mode))
  qlen = tf.shape(q)[2]
  klen = tf.shape(k)[2]
  vlen = tf.shape(v)[2]
  num_heads = q.get_shape().as_list()[1]
  ...
  dim = q.get_shape().as_list()[3]
  kdim = k.get_shape().as_list()[3]

  # _k =  common_layers.dense(_k, dim,
    #                   use_bias=False, name="compress")
  # init = tf.random_normal_initializer(
  #     0, dim**-0.5)
  output_depth = v.get_shape().as_list()[3]

  bsz = tf.shape(q)[0]
  pq = tf.get_variable('pq',[num_heads, num_slots, dim],
                    initializer=tf.random_normal_initializer(
                        0, output_depth**-0.5 * (num_heads**0.5)))
  pk = tf.get_variable('pk',[num_heads, num_slots, dim],
                    initializer=tf.random_normal_initializer(
                        0, output_depth**-0.5 * (num_heads**0.5)))

  k = tf.transpose(k, [0, 2, 1, 3])
  v = tf.transpose(k, [0, 2, 1, 3])
  k = tf.reshape(k, [num_heads, bsz * klen, kdim])
  v = tf.reshape(v, [num_heads, bsz * vlen, kdim])
  q = tf.transpose(q, [0, 2, 1, 3])
  q = tf.reshape(q, [num_heads, bsz * qlen, kdim])
  print(dropout_broadcast_dims)
  print(bias)
  blen = tf.shape(bias)[-1]
  bias = tf.reshape(bias, [1, 1, bsz * blen])
  a = dot_product_attention(pq * (dim ** -0.5), k, v, bias,
                              dropout_rate, image_shapes,
                              save_weights_to=save_weights_to,
                              make_image_summary=False,
                              dropout_broadcast_dims=dropout_broadcast_dims)
  # a - bsz x num_heads x num_slots x dim
  b = dot_product_attention(pk * (dim ** -0.5), k, v, bias,
                              dropout_rate, image_shapes,
                              save_weights_to=save_weights_to,
                              make_image_summary=False,
                              dropout_broadcast_dims=dropout_broadcast_dims)
  x = dot_product_attention(q * (dim ** -0.5), a, b, None,
                              dropout_rate, image_shapes,
                              save_weights_to=save_weights_to,
                              make_image_summary=False,
                              dropout_broadcast_dims=dropout_broadcast_dims)
  x = tf.reshape(x, [num_heads, bsz, qlen, kdim])
  x = tf.transpose(x, [1, 2, 0, 3])
  x = tf.reshape(x, [bsz, num_heads, qlen, kdim])
  return x


def linear_attention_v3(q,
                        k,
                        v,
                        bias,
                        dropout_rate=0.0,
                        ...
                        conditioned=False,
                        conv=False):
  print("Linear Attention V3 {} {}".format(simple, att_mode))
  qlen = tf.shape(q)[2]
  klen = tf.shape(k)[2]
  vlen = tf.shape(v)[2]
  num_heads = q.get_shape().as_list()[1]
  ...
  # _k =  common_layers.dense(_k, dim,
    #                   use_bias=False, name="compress")
  init = tf.random_normal_initializer(0, dim ** -0.5)
  context = tf.get_variable('context_key',[1, num_heads, dim, num_slots],
                    initializer=init)
  bsz = tf.shape(q)[0]
  context = tf.tile(context, [bsz, 1, 1, 1])

  q_softmax = tf.matmul(q, context)    # bsz x num-heads, seq_len, num_slots
  q_softmax = tf.reshape(q_softmax, [-1, num_heads, num_slots, qlen])

  q_softmax = tf.nn.softmax(q_softmax)

    # k is bsz x num_heads, seq_len, dim
  q2 = tf.expand_dims(q, 2)     # bsz x num_heads x 1 x seq_len x dim
  q2 = tf.tile(q2, [1, 1, num_slots, 1, 1])

  q_softmax = tf.expand_dims(q_softmax, 4)

  q2 = q2 * q_softmax

  q2 = tf.reduce_sum(q2, 3)

  xv = dot_product_attention(q2 * (dim ** -0.5), k, v, bias,
                          dropout_rate, image_shapes,
                          save_weights_to=save_weights_to,
                          make_image_summary=make_image_summary,
                          dropout_broadcast_dims=dropout_broadcast_dims)
  xk = dot_product_attention(q2 * (dim ** -0.5), v, k, bias,
                          dropout_rate, image_shapes,
                          save_weights_to=save_weights_to,
                          make_image_summary=make_image_summary,
                          dropout_broadcast_dims=dropout_broadcast_dims)
  x = dot_product_attention(q * (dim ** -0.5), xk, xv, None,
                          dropout_rate, image_shapes,
                          save_weights_to=save_weights_to,
                          make_image_summary=make_image_summary,
                          dropout_broadcast_dims=dropout_broadcast_dims)
